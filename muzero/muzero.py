...
class MuZero:
    ...
    def __init__(
        self, game_name, config=None, split_resources_in=1, write_config=False
    ):
        ...
        try:
            ...
            if game_name == "rlp":
                ...
                print(f"Loaded {config['puzzle']}")

            else:
                self.Game = game_module.Game
            if game_name == "rlp":
                self.config = game_module.MuZeroConfig(self.Game().env)
            else:
        ...
        if config:
            if type(config) is dict:
                ...
                self.config = config
        # Store config as textfile in the results folder
        self.config.results_path = pathlib.Path(self.config.results_path)
        print(f"Result path {self.config.results_path}")
        if write_config:
            self.config.save()
            print("Saved config")
    ...
    def train(self, log_in_tensorboard=True):
        ...
        if log_in_tensorboard or self.config.save_model:
            pathlib.Path(self.config.results_path).mkdir(parents=True, exist_ok=True)
        ...
        self.shared_storage_worker.set_info.remote("nr_tests", 0)
    ...
    def test_multihread(
        ...
        for i, worker in enumerate(workers):
            future = worker.play_game.remote(render, opponent, muzero_player)
            futures.append(future)
            future_to_worker[future] = i
        all_rewards = []
        all_episode_lengths = []
        while len(all_rewards) < num_tests:
            done_id, futures = ray.wait(futures)
            result = ray.get(done_id[0])
            all_rewards.append(sum(result.reward_history))
            all_episode_lengths.append(len(result.action_history) - 1)
            del result

            completed_worker_index = future_to_worker.pop(done_id[0])

            if len(all_rewards) < num_tests:
                # Submit a new game for the worker that just completed
                new_future = workers[completed_worker_index].play_game.remote(
                    render, opponent, muzero_player
                )
                futures.append(new_future)
                future_to_worker[new_future] = completed_worker_index
            print(f"Testing {len(all_rewards)}/{num_tests}")

        all_rewards = numpy.array(all_rewards)
        all_episode_lengths = numpy.array(all_episode_lengths)

        # also store these arrays in a file
        numpy.save(os.path.join(results_dir, "all_rewards.npy"), all_rewards)
        numpy.save(
            os.path.join(results_dir, "all_episode_lengths.npy"), all_episode_lengths
        )

        # Calculate average and standard deviation
        average_reward = numpy.mean(all_rewards)
        average_episode_length = numpy.mean(all_episode_lengths)
        average_success_episode_length = numpy.mean(
            all_episode_lengths[all_rewards > 0]
        )
        std_dev_episode_length = numpy.std(all_episode_lengths)
        success_rate = numpy.where(all_rewards > 0, 1.0, 0.0).mean()
        ...
        print(
            f"\nAverage reward: {average_reward}, Average episode length: {average_episode_length}, Std Dev Episode Length: {std_dev_episode_length}\n"
        )
    ...
    def test(
        ...
        print(
            f"\nAverage reward: {result}, Average episode length: {average_episode_length}\n"
        )
        return result
...
def load_config(file_path):
    ...
    with open(file_path, "r") as file:
        for line in file:
            ...
            try:
                # Safely evaluate value if it's a literal (e.g., list, tuple)
...
if __name__ == "__main__":
    print(f"Detected {len(sys.argv)} args")
    if len(sys.argv) > 1 and sys.argv[1] == "test":
        print("Testing")
        # load config dict from txt file
        config_path = os.path.join(sys.argv[2], "config.txt")
        config = load_config(config_path)
        print(f"Loaded config from {config_path}")

        muzero = MuZero("rlp", config, write_config=False)
        path = config["results_path"]
        muzero.load_model(os.path.join(path, "model.checkpoint"))
        ...
        print(f"Detected 3 sysargs, training directly with args {sys.argv[2]}")
        config = json.loads(sys.argv[2])
        muzero = MuZero(sys.argv[1], config, write_config=True)
        muzero.train()
    else:
